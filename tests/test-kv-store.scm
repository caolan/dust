...
(define (with-test-envs paths thunk #!optional (envs '()))
  (if (null? paths)
      (apply thunk envs)
      (with-test-env
       (car paths)
       (lambda (env)
         (with-test-envs (cdr paths) thunk (cons env envs))))))
...
(define (with-test-store path thunk)
  (with-test-env
   path
   (lambda (env)
     (with-txn env (compose thunk kv-store-open)))))
